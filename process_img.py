...
def crop_image(img):
    ...
    iContours = 0
    for contour in cnts:
        cv2.drawContours(img_canny, cnts, iContours, (0, 255, 0))
        iContours += 1
    ...
    if len(cnts) > 0:
        ...
        print('get_x_ver1',get_x_ver1)

        # loop over the sorted contours
        for i, c in enumerate(cnts):
            x_curr, y_curr, w_curr, h_curr = cv2.boundingRect(c)

            print('diện tích:', w_curr * h_curr )
